...
fn main() {
    ...
    let collapse_count = samples.iter().filter(|c| {
        if let Err(ref e) = c {
            let s = format!("{e}");
            s.contains("too close") || s.contains("identical")
        } else { false }
    }).count();
    let mass_count = samples.iter().filter(|c| {
        if let Err(ref e) = c {
            format!("{e}").contains("mass")
        } else { false }
    }).count();
    ...
    output.push_str(&format!("Valid configurations:     {valid_count} ({:.2}%)\n", fraction * 100.0));
    output.push_str(&format!("Collision/collapse:      {collapse_count} ({:.2}%)\n", collapse_count as f64 / N_SAMPLES as f64 * 100.0));
    output.push_str(&format!("Non-positive mass:       {mass_count} ({:.2}%)\n", mass_count as f64 / N_SAMPLES as f64 * 100.0));
    ...
    output.push_str(&format!("  Stability reduction:           {:.2}× = {:.1} bits\n", fraction.max(1e-10).recip(), -(fraction.max(1e-10)).log2()));
    output.push_str(&format!("  Estimated H:                   {estimated_bits:.0} bits\n"));
    output.push_str(&format!("  Claimed bound:                 ≥ 1920 bits\n\n"));
    output.push_str(&format!("Grover search lower bound:   2^{grover_bits:.0} quantum operations\n"));
    ...
    if estimated_bits >= 1920.0 {
        ...
        output.push_str(&format!("⚠ Estimated H ({estimated_bits:.0}) below 1920 bound — check analytical model\n"));
    }
