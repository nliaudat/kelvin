@echo off
REM ==============================================================================
REM Kelvin -- 100 GB Encryption Benchmark (Windows Batch)
REM ==============================================================================
REM Benchmarks all 4 modes (secure, chaos, photon, quantum) on a 100 GB file
REM with alternating 0x00/0x01 pattern. Outputs results to documentation/100gb_benchmark.md
REM
REM Usage:
REM   scripts\bench-100gb.bat
REM ==============================================================================
...
set INPUT_FILE=%TEMP_DIR%\test_100gb.bin
set ENC_FILE=%TEMP_DIR%\test_100gb.enc
set DEC_FILE=%TEMP_DIR%\test_100gb_dec.bin
set REPORT=documentation\100gb_benchmark.md

echo %CYAN%========================================%NC%
echo %CYAN%  Kelvin 100 GB Encryption Benchmark%NC%
echo %CYAN%========================================%NC%
echo.
...
echo %CYAN%[4/6] Generating 100 GB alternating pattern file...%NC%
echo %YELLOW%  Creating 100 GB zero file with fsutil...%NC%
REM fsutil creates a zero-filled file instantly
fsutil file createnew "%INPUT_FILE%" 107374182400
if errorlevel 1 (
...
REM Write Python script to temp file to avoid cmd multiline issues
echo import sys > "%TEMP_DIR%\write_pattern.py"
echo buf = bytearray(1048576) >> "%TEMP_DIR%\write_pattern.py"
echo for i in range(0, 1048576, 2): >> "%TEMP_DIR%\write_pattern.py"
echo     buf[i] = 0x00 >> "%TEMP_DIR%\write_pattern.py"
echo     if i + 1 ^< 1048576: >> "%TEMP_DIR%\write_pattern.py"
echo         buf[i+1] = 0x01 >> "%TEMP_DIR%\write_pattern.py"
echo with open(r'%INPUT_FILE%', 'r+b') as f: >> "%TEMP_DIR%\write_pattern.py"
echo     for i in range(102400): >> "%TEMP_DIR%\write_pattern.py"
echo         f.write(buf) >> "%TEMP_DIR%\write_pattern.py"
echo         if i % 10 == 0: >> "%TEMP_DIR%\write_pattern.py"
echo             import sys; sys.stderr.write('.'); sys.stderr.flush() >> "%TEMP_DIR%\write_pattern.py"
echo import sys; sys.stderr.write('\n') >> "%TEMP_DIR%\write_pattern.py"
...
echo # Kelvin 100 GB Encryption Benchmark > "%REPORT%"
echo. >> "%REPORT%"
echo **Date:** %DATE% %TIME% >> "%REPORT%"
echo **Platform:** Windows >> "%REPORT%"
echo **Test file:** 100 GB alternating 0x00/0x01 pattern >> "%REPORT%"
echo **Integration:** Euler (default) >> "%REPORT%"
echo **Key level:** standard (5 bodies, 1M steps) >> "%REPORT%"
echo. >> "%REPORT%"
echo ^| Mode ^| Operation ^| Time ^(s^) ^| Throughput ^(GB/s^) ^| Verify ^| >> "%REPORT%"
echo ^|---^|-----------^|----------^|-------------------^|--------^| >> "%REPORT%"

REM Note: 'secure' mode requires Lyapunov chaos validation which may fail
REM with some generated keys. 'chaos' mode (V2 streaming) has no such check.
...
for %%m in (%MODES%) do (
    ...
    if not "!ENC_TIME!"=="ERROR" (
        python -c "print('%.3f' % (100.0 / !ENC_TIME!))" > %TEMP_DIR%\enc_gbps.txt
        set /p ENC_GBPS=<%TEMP_DIR%\enc_gbps.txt
    ) else (
        set ENC_GBPS=0
    )
    if not "!DEC_TIME!"=="ERROR" (
        python -c "print('%.3f' % (100.0 / !DEC_TIME!))" > %TEMP_DIR%\dec_gbps.txt
